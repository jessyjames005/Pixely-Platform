...
const { loading, error, execute: fetchExtensions } = useApi(extensionsStore.fetchExtensions)
const { loading: toggling, execute: toggleEnable } = useApi(extensionsStore.enable)
...
const { loading: installing, error: installError, execute: submitInstall } = useApi(extensionsStore.install)

// Update dialog
const updateDialogOpen = ref(false)
const updateTargetId = ref<string | null>(null)
const updateFile = ref<File | File[] | null>(null)
const { loading: updating, error: updateError, execute: submitUpdate } = useApi(extensionsStore.update)

// Config dialog
const configDialogOpen = ref(false)
const configTargetId = ref<string | null>(null)
const configText = ref('{}')
const { loading: loadingConfig, execute: fetchConfig } = useApi(extensionsStore.fetchConfig)
const { loading: savingConfig, error: configError, execute: submitConfig } = useApi(extensionsStore.updateConfig)

// Details dialog
const detailsDialogOpen = ref(false)
const detailsData = ref<ExtensionDetail | null>(null)
const { loading: loadingDetails, execute: fetchDetail } = useApi(extensionsStore.fetchDetail)
...
<template>
  <div>
    <div class="d-flex align-center justify-space-between mb-4">
      ...
      <v-btn color="primary" prepend-icon="mdi-plus" @click="openInstallDialog">Install extension</v-btn>
    </div>
      ...
      <v-card-title class="d-flex align-center justify-space-between">
        ...
        <div class="d-flex align-center ga-4">
          <v-switch
            ...
            hide-details
          />
          <v-btn size="small" variant="tonal" :loading="loading" @click="fetchExtensions">Refresh</v-btn>
        </div>
      </v-card-title>

      <v-alert v-if="error" type="error" density="compact" class="mx-4">{{ error.message }}</v-alert>

      <v-data-table :headers="headers" :items="extensionsStore.extensions" :loading="loading" item-value="id">
        <template #no-data>
          <p class="text-medium-emphasis py-6">No extensions installed.</p>
        </template>

        <template #item.dependencies="{ item }">
          <v-chip v-for="dep in item.dependencies" :key="dep" size="small" class="mr-1">{{ dep }}</v-chip>
          <span v-if="item.dependencies.length === 0" class="text-medium-emphasis">—</span>
        </template>
        ...
        <template #item.actions="{ item }">
          <v-btn icon="mdi-eye" size="small" variant="text" @click="openDetailsDialog(item)" />
          <v-btn icon="mdi-cog" size="small" variant="text" @click="openConfigDialog(item)" />
          <v-btn icon="mdi-upload" size="small" variant="text" @click="openUpdateDialog(item)" />
          <v-btn
            icon="mdi-delete"
            size="small"
            variant="text"
    ...
    <v-dialog v-model="installDialogOpen" max-width="480" persistent>
      <v-card title="Install a new extension">
        <v-card-text>
          <v-file-input v-model="installFile" label="Extension package (.zip)" accept=".zip" :disabled="installing" />
          <v-alert v-if="installError" type="error" density="compact" class="mt-2">{{ installError.message }}</v-alert>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="installDialogOpen = false">Cancel</v-btn>
          <v-btn color="primary" :loading="installing" :disabled="!getSelectedFile(installFile)" @click="handleInstall">
            Install
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="updateDialogOpen" max-width="480" persistent>
      <v-card :title="`Update ${updateTargetId}`">
        <v-card-text>
          <v-file-input v-model="updateFile" label="New package (.zip)" accept=".zip" :disabled="updating" />
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="updating" :disabled="!getSelectedFile(updateFile)" @click="handleUpdate">
            Update
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="configDialogOpen" max-width="600" persistent>
      <v-card :title="`Configuration — ${configTargetId}`">
        <v-card-text>
          ...
          <v-textarea v-else v-model="configText" rows="12" font="monospace" label="Configuration (JSON)" />
          <v-alert v-if="configError" type="error" density="compact" class="mt-2">{{ configError.message }}</v-alert>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="configDialogOpen = false">Cancel</v-btn>
          <v-btn color="primary" :loading="savingConfig" @click="handleSaveConfig">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-card :title="`Details — ${detailsData?.id ?? ''}`">
        <v-card-text>
          ...
          <v-list v-else-if="detailsData" density="compact">
            ...
            <v-list-item title="Enabled" :subtitle="detailsData.enabled ? 'Yes' : 'No'" />
            <v-list-item title="Dependencies" :subtitle="detailsData.dependencies.join(', ') || 'None'" />
            <v-list-item title="Service providers">
              <v-list-item-subtitle v-for="provider in detailsData.providers" :key="provider">
                {{ provider }}
