...
} = useApi(usersStore.createUser);
const {
  loading: updating,
  error: updateError,
  execute: submitUpdate,
} = useApi(usersStore.updateUser);
const { loading: deleting, execute: removeUser } = useApi(
  usersStore.deleteUser,
);
const { loading: assigning, execute: submitAssign } = useApi(
      ...
      >
        ...
        <template #item.role="{ item }">
          <v-select
            ...
            :disabled="assigning"
            @update:model-value="
              (value: string | null) => handleAssignRole(item.id, value)
            "
          />
        </template>
        ...
        <template #item.actions="{ item }">
          ...
          />
          <v-btn
            v-if="authStore.can('users.delete')"
            icon="mdi-delete"
            size="small"
            variant="text"
            color="error"
